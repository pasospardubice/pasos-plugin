 ...
 */
add_filter( 'manage_edit-klub_sortable_columns', 'manage_wp_posts_be_qe_manage_sortable_columns' );
 ...
 */
add_action( 'manage_klub_posts_custom_column', 'manage_wp_posts_be_qe_manage_posts_custom_column', 10, 2 );
	...
	if ( $query->is_main_query() && ( $orderby = $query->get( 'orderby' ) ) ) {
		...
		switch( $orderby ) {
			...
			case 'home_klub_lose':
				...
				$query->set( 'meta_key', 'home_klub_lose' );
	...
	if ( $query->is_main_query() && ( $orderby = $query->get( 'orderby' ) ) ) {
		...
		switch( $orderby ) {
			...
			case 'klub_sezona':
				 ...
				 */
				$pieces[ 'join' ] .= " LEFT JOIN $wpdb->postmeta wp_rd ON wp_rd.post_id = {$wpdb->posts}.ID AND wp_rd.meta_key = 'klub_sezona'";
